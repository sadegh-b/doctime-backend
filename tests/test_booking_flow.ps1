...
Write-Step "Step 5 - Extract first created slot id"

$slotId = $null

if ($availability.data -and $availability.data.slots -and $availability.data.slots.Count -gt 0) {
    $slotId = $availability.data.slots[0].id
}
elseif ($availability.slots -and $availability.slots.Count -gt 0) {
    $slotId = $availability.slots[0].id
}
elseif ($availability.data -and $availability.data.id) {
    $slotId = $availability.data.id
}
elseif ($availability.id) {
    $slotId = $availability.id
}

if (-not $slotId) {
    Write-Fail "Could not extract slot id from availability response"
    Write-Host "Availability response was:" -ForegroundColor Yellow
    $availability | Print-Json
    exit 1
}
